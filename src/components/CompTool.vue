<template>
  <div id="comp_room">
    <div style="position: relative; display: block;" id="tablewrap" class="create">
      <table>
        <tbody>
          <tr v-for="(listItem,i) in comp_room" :key="i">
    ...
    <textarea placeholder="Notizen" v-model="note" id="noteblock" cols="30" rows="10"></textarea>
...
export default {
  ...
  methods: {
    updateView() {
      ...
      axios
        .get("/php/getCompInfo.php?tool_id=" + obj.$route.params.tool_id, {})
        .then(function(response) {
          obj.comp_room = response.data.data;
          obj.note = response.data.note;
          var obj2 = obj;
          axios
            .get(
    ...
    removeChampion(i, o) {
      this.comp_room[i].champions.splice(o, 1);
      this.saveState();
    },
    addChampion(index) {
      var exists = false;
      this.comp_room[index].champions.forEach(champ => {
        if (champ.name == this.selected.name) {
          exists = true;
        }
      });
      if (!exists) {
        this.comp_room[index].champions.push(this.selected);
        this.saveState();
      } else {
        alert(this.selected.name + " exestiert schon in dieser Reihe.");
      }
    },
    saveState() {
      var obj = this;
      axios
        .get(
          "/php/saveCompRoom.php?tool_id=" +
            obj.$route.params.tool_id +
            "&new_list=" +
            JSON.stringify(obj.comp_room)+
            "&note="+obj.note,
          {}
        )
        .then(function() {})
